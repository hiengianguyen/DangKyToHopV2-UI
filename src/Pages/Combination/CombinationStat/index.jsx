            ...
            <div className="flex justify-center flex-col">
                ...
                <Pc>
                    <div className="flex gap-8">
                        <StartCard value={dataChart.length} title="Tổng hồ sơ nộp" color="bg-blue-500" subText={`+36 hồ sơ hôm nay`} icon={<svg className="w-18 h-18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>}/>
                        <StartCard value={dataChart.submittedGoal} title="Chỉ tiêu tuyển sinh" color="bg-purple-500" subText={`240% so với chỉ tiêu`} icon={<svg className="w-18 h-18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>}/>
                        <StartCard value={dataChart.approvedLength} title="Tổng hồ sơ nộp" color="bg-emerald-500" subText={`36% chỉ tiêu đã đạt`} icon={<svg className="w-18 h-18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}/>
                        <StartCard value={dataChart.submittedLength} title="Tổng hồ sơ nộp" color="bg-orange-400" subText={`Cần xem xét ngay`} icon={<svg className="w-18 h-18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}/>
                    </div>
                </Pc>
                <Desktop>
                        ...
                        <div className="flex gap-8 mb-24">
                                ...
                                <div className="rounded-xl bg-white shadow-sm border p-6 border-gray-50 flex flex-col justify-center h-100 w-max">
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        <div className="flex flex-col ms-4 gap-4 w-max">
                                            <Progress title={'Đã chỉnh sửa'} subText={'Chỉnh sửa tỉ mỉ, đúng đắn.'} value={dataChart.isEdited} max={dataChart.length} color={'#10b981'}/>
                                            <Progress title={'Chưa chỉnh sửa'} subText={'Nộp ngay lần đầu'} value={dataChart.length - dataChart.isEdited} max={dataChart.length} color={'#f59e0b'}/>
                                        </div>
                                    </div>
                                </div>
                                <div className="rounded-xl bg-white shadow-sm border p-6 border-gray-50 flex flex-col justify-center h-100 w-max">
                                    ...
                                    <div className="flex flex-col items-center justify-center gap-10 w-100">
                                        <div className="rounded-full position-relative size-56 flex justify-center items-center text-p hover:rotate-6 cursor-pointer transition-transform" style={{background: `conic-gradient(#3b82f6 0% ${((dataChart.maleGender/dataChart.length)*100).toFixed(0)}%, #ec4899 ${((dataChart.maleGender/dataChart.length)*100).toFixed(0)}% 100%)` }}>
                                        ...
                                        <div className="flex gap-4">
                                            <CommentGender title={'Nam (' + ((dataChart.maleGender/dataChart.length)*100).toFixed(0) + '%)'} color={"#3b82f6"}/>
                                            <CommentGender title={'Nữ (' + (((dataChart.length - dataChart.maleGender)/dataChart.length)*100).toFixed(0) + '%)'} color={"#ec4899"}/>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="rounded-xl bg-white shadow-sm border p-6 border-gray-50 w-100">
                                ...
                                <div className="flex flex-col gap-3">
                                    {dataChart.schoolCount && dataChart.schoolCount.map((item, index) => (
                            ...
                            <div className="rounded-xl bg-white shadow-sm border p-6 border-gray-50 w-100">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="bg-indigo-200 text-indigo-600 rounded-lg fw-bold p-3 fs-4" title={"Trung bình: " + dataChart.avgPoint}>TB: {dataChart.avgPoint}</span>
                                </div>
                                <div className="flex gap-2 h-4/5">
                                    {dataChart.pointCount && dataChart.pointCount.map((item, index) => (
