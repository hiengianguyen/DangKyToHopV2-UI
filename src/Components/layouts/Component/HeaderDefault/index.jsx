...
function HeaderDefault() {
  ...
  const mainRouters = useMemo(() => [...publicRoutes, ...privateRoutes], []);
  ...
  useEffect(() => {
    setMenuRoutes(
      mainRouters.filter((route) => {
        return (
          route.inSidebar !== false && route.roles.includes(auth?.user?.role)
        );
      })
    );
  }, [mainRouters, auth?.user?.role]);
  ...
  return (
    <div className={cx("d-flex flex-column", "wrapper")}>
      <div className={cx("header")}>
        <div className={cx("d-flex", "align-items-center", "item-header")}>
          ...
          <div className={cx("tittle-sidebar")}>
            <span className={cx("name", "fw-bolder fs-2")}>Đăng ký tổ hợp</span>
            <span className={cx("des", "fw-medium")}>Trường THPT Duy Tân</span>
          </div>
          <div
            className={
              cx("nav-bar-icon", "main") +
        ...
        <div className={cx("links")}>
          {menuRoutes &&
            menuRoutes.map((item, index) => (
              <Link
                to={item.path}
                className={cx({ active: url === item.path })}
                key={index}
              >
